...
export abstract class BasePage<T> {
  ...
  setRecordData() {
    ...
    this.submitedData.record.id = AppCommon.newModelId();
  }
  /** 设置照片信息 */
  setMediaData() {
    ...
    _.each(this.medias, (n, i) => {
      ...
      //ext = "jpg"
      n.fileName = record.id + "-" + this.currentUser.id + "-" + n.ix + "." + ext;
  ...
  /** 获取线路 */
  getRoads() {
    return this.dbService.query("select * from road where CompanyID = ? ", [this.currentUser.companyId])
      .then(items => {
        this.roads = items;
        return items;
      });
